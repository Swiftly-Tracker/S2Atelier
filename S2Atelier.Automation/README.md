...
`CS2-Dumps/.github/workflows/update.yml` 调用 `Dumps/.github/workflows/dumper.yml`，取得 **实际推送的提交 SHA**，再调用 `Dumps/.github/workflows/trigger_update.yml`。后者通过 HTTP 提交 `{ "dumpsCommit": "<40-character SHA>", "platform": "all" }`。Actions 提交后即结束，VM 串行完成 Windows/Linux 两个平台，最长 72 小时。
...
3. 按模块编译 protobuf，导入 protobuf/schema/HL2SDK 接口，运行 ConVar、函数指针、PLT 等分析。容器无网络，限制为 2 CPU、4 GiB 内存，一次解析一个文件。
